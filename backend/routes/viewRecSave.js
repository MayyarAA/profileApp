...
module.exports = async ( manuID, assetId, recValue, databaseConnection, res) => {
    let Request = require('tedious').Request;
    let TYPES = require('tedious').TYPES;

    //this backend call will update the asset and user row with the 
    //corresponding recency rating the user inputed and the current timestamp
    let sql = "update asset_skills_table set recency = @rValue ,confirm_time = CURRENT_TIMESTAMP from asset_skills_table ak inner join assets_table aj on ak.Asset_ID = aj.Asset_ID where Manulife_ID = @manuID and Asset_Name = @assetId"
    ...
    request.addParameter('rValue', TYPES.VarChar, recValue);

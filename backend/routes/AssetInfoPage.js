...
module.exports = async (assetName, databaseConnection, res) => {
    let sql = "select asset_id, asset_description from assets_table where asset_name = @name";
    let request = new Request (sql, function(err, rowCount, rows) { });
    
    let assetId, assetDesc;
    request.on('row', function (columns) {
        columns.forEach(function (column) {
            if(column.metadata.colName == "asset_id") {
                assetId = column.value
            }
            if(column.metadata.colName == "asset_description") {
                assetDesc = column.value
            }
    ...
    request.on('requestCompleted', async function() {
        await getAssetSkillsInformation(assetId, assetDesc, databaseConnection, res);
    });
...
async function getAssetSkillsInformation(assetId, assetDesc, databaseConnection, res) {
    ...
    request.on('requestCompleted', function() {
        ...
        res.json({employeeIds: manulifeIds, employeeTechRatings: technicalRatings, employeeBusRatings: businessRatings, employeeRecencies: recencies, assetIdentification: assetId, assetDescription: assetDesc});
    });
